...
class InstagramClient(object):
    ...
    def getSelfPost(self):
        return self.getPosts(self.api.username)

    def getPosts(self, username):
        list = []
        usernameid = self.get_userID(username)
        self.api.getUserFeed(usernameid)
    ...
    def uploadPhoto(self, photo, message):
        print("Now Uploading this photo to instagram: " + message)
        urllib.request.urlretrieve(photo, 'temp.jpg')
        self.api.uploadPhoto('temp.jpg', message)

    def uploadPhotoList(self, list):
        for l in list:
            self.uploadPhoto(l[2], l[0])
            n = randint(5, 10)
